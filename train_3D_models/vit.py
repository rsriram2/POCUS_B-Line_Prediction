...
# Load training data
train_mat_file_paths = glob.glob('/dcs05/ciprian/smart/pocus/rushil/bounding_box_stacked_data/training/*/*.mat')
train_dataset = VolumeDataset(train_mat_file_paths, transform)
train_dataloader = DataLoader(train_dataset, batch_size=12, shuffle=True)

# Load validation data
val_mat_file_paths = glob.glob('/dcs05/ciprian/smart/pocus/rushil/bounding_box_stacked_data/validation/*/*.mat')
...
model = ViT(
    image_size=(256, 384),     # spatial size of the input images
    frames=45,                 # number of slices per file
    image_patch_size=16,       # size of patches to divide images
    frame_patch_size=15,        # size of patches for frames (45 slices / 5 = 9 frame patches)
...
def validate(model, val_dataloader):
    ...
    with torch.no_grad():  # No need to compute gradients during validation
        for volumes, labels in val_dataloader:
            ...
            volumes = volumes.permute(0, 2, 1, 3, 4)  # [batch, channels, height, width, depth]
...
writer = SummaryWriter(log_dir='/dcs05/ciprian/smart/pocus/rushil/vit_model_logs')
for epoch in range(num_epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), '/dcs05/ciprian/smart/pocus/rushil/vit_paths/vit.pth')
